...
param([string]$VMhost=$_)
    ...
    $sshenable = get-VMHostService -VMHost $VMhost | where{$_.Key -eq 'TSM-SSH'} | Start-VMHostService -Confirm:$false
...
function checkSLP
{
    ...
    (
        ...
        $session = New-SSHSession -ComputerName $VMhost -Credential $Credential
...
[CmdletBinding()]
    ...
    {
        $read = read-host "Credentials were not authenticated. Please press 1 to abort this host or press enter for password prompt:"
        if($read -eq 1)
        {
            return 1
        }
...
foreach($VMhost in $hosts)
{
    ...
    while($validated -ne $true)
    {
        ...
        $validated = validateESXiCred -VMhost $VMhost.name -Credential $cred2         
